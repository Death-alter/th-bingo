...
// import { VuexState, StoreData, ActionHandler, MutationHandler, RequestParams} from "@/types";
...
// export const createSyncMutation = (name: string, callback:MutationHandler ) => (state: VuexState, data:RequestParams) => {
...
//   (name: string, actionName: string, url: string, method: string, callback: ActionHandler) =>
//   ({ commit, state }, data:RequestParams) =>
...
//           .then((res) => {
//             if (callback) {
//               res = callback(res, state[name].data, data);
//             }
